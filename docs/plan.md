# StayTube: Project Plan

## 1. Overview
The goal is to build StayTube, a cross-platform desktop application (Linux, Windows, macOS) using Electron that serves as a modern, user-friendly frontend for the `yt-dlp` CLI tool. The application will allow users to download videos from YouTube (and other supported sites), select video qualities, and manage subtitles.
